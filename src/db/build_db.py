...
class Control:
    """Aggregates all needed Controls to run"""
    ...
    def get_test_filename():
        ...
        suffix = f'-{Control.cfg.font_family}-{Control.cfg.font_size}' # type: ignore
        return os.path.join(Control.base_dir,"src/db/test"+suffix+".html")
    @staticmethod
    ...
    def run(cfg):
        Control.cfg = cfg
        Control.cursor = LineCursor(0, 0)
        Control.json_helper = JsonHelper(cfg)
        Control.html_helper = HtmlHelper()
        ...
        Control.html_helper.make_html_test()
        #
        # Finally Load the Html Template and Driver
        chrome_options = Options()
        chrome_options.add_experimental_option("excludeSwitches", ['enable-automation'])
        Control.web_driver = webdriver.Chrome(options=chrome_options)
        test_url = "file://" + Control.get_test_filename()
        Control.web_driver.get(test_url)
        Control.html_helper.ensure_page_has_loaded(test_url)
        suras = []
        print("Processing ..")
        with tqdm.tqdm(total=os.path.getsize(os.path.join(TMP,TXT)), leave=False) as progress_bar:
            Control.progress = progress_bar
            with open(os.path.join(TMP,TXT), encoding="utf8") as file_txt:
                suras = Mushaf(Control.cfg, file_txt).process()
        print("Closing Chrome ..")
        Control.web_driver.close()
        os.remove(Control.get_test_filename())
        Control.json_helper.save_json(suras)
        Control.print_dbg_widths()

class LineCursor:
    """Part of the global Control, points to the current point in DB"""
...
class Ayah(Control):
    ...
    def process(self):
        ...
        if len(ayah_data) != len(self.text.split())-skip_words:
            print(f'Mismatch at {self.sura_index}-{self.index}: '
        ...
        for line_index, line in words_in_lines.items():
            ...
            aya_part_width = Control.html_helper.get_width(aya_text_part)
        ...
        Control.progress.update(len(self.text.encode('utf-8'))+2)
...
class Surah:
    ...
    def get_surah_name(sura_id, decorate = False):
        ...
        return sura_name
    def get_aya01(self):
        page = Control.cursor.page
        line = Control.cursor.line
        ...
        aya01 = [{ "p": title_page,
                   "r": [{"l": title_line,
                          "t": Surah.get_surah_name(self.index, True), "o": 0, "s": -1}]},
                { "p": bsm_page,
...
class HtmlHelper:
    ...
    @staticmethod
    def make_html_test():
        font = Control.cfg.font_family
        font_url = Control.cfg.font_url
        font_sz = Control.cfg.font_size
        line_width = Control.cfg.line_width
        ...
        with open(Control.get_test_filename(), 'w', encoding="utf8") as file:
            file.write(str(soup))
    @staticmethod
    def update_html_text(text):
        Control.web_driver.execute_script(f'document.getElementById(\'test\')' \
                                          f'.textContent = \'{text}\'')
    @staticmethod
    def ensure_page_has_loaded(url):
        time.sleep(5) # Hack!
        try:
            WebDriverWait(Control.web_driver, 5).until(EC.url_to_be(url))
    ...
    def get_width(aya_text):
        ...
        return Control.web_driver.execute_script("return document.getElementById('test')"
